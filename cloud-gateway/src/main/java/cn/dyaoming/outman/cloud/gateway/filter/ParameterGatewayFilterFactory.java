...
@Component
@Slf4j
public class ParameterGatewayFilterFactory extends BaseGatewayFilterFactory {

	//提取版本号正则
	String regVersion = "(?<=/)[v|V]([1-9]\\d*\\.\\d*|0\\.\\d*[1-9]\\d*|\\d+)?(?=/)";
	//提取接口编码正则
	String regInterfaceCode = "(?<=/[v|V])([^/]*)/([^/]*)";
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		Matcher param = Pattern.compile(regInterfaceCode).matcher(url);
		param.find();
		String parameter = param.group();
		System.out.println("version:" + parameter.split("/")[0]);
		System.out.println("interfaceCode:" + parameter.split("/")[1]);
